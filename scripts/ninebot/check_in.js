 ...
 * 九号出行签到
 * @author emoemperor
 * @homepage https://github.com/emoemperor/Surge/scripts/ninebot
 * @icon https://raw.githubusercontent.com/emoemperor/Surge/master/scripts/ninebot/logo.png
 * @date 2026-03-11
 * 
 * 
Surge
[Script]
# 九号出行签到
九号出行签到 = type=cron,cronexp=0 7 * * *,wake-system=1,script-path=https://raw.githubusercontent.com/emoemperor/Surge/master/scripts/ninebot/check_in.js
 * 
 * 
 * */
...
const deviceIdKey = `${prefix}ninebot_device_id`;

!(async () => {
    const cookie = $.getval(cookieKey);
    const deviceId = $.getval(deviceIdKey);
    if (!cookie || !deviceId) {
        $.msg(NAME, "⚠️请先获取cookie和device_id");
        return;
    }
    await check_in(cookie, deviceId);
})()
            ...
            try {
                ...
                if (result.code === 0) {
                    if (data.data.rewardList && data.data.rewardList.length > 0) {
                        let rewards = data.data.rewardList.map(reward => {
                            let rewardName = "";
                            ...
                            return `${reward.rewardValue}${rewardName}`;
                        }).join("\n");
                        $.msg(NAME, "✅签到成功", `获得\n${rewards}`);
                    } else {
                        $.msg(NAME, "✅签到成功", `响应数据`, JSON.stringify(result.data));
                    }
                } else {
                    $.msg(NAME, "❌签到失败", result.message);
                }
            } catch (e) {
                $.logErr(e);
                $.msg(NAME, "❌签到失败", "无法解析响应");
            }
        }
    });
}
