 ...
 * 蔚来签到
 * @author emoemperor
 * @homepage https://github.com/emoemperor/Surge/scripts/nio
 * @date 2026-03-11
 *
 * Surge
 * [Script]
 * # 蔚来签到
 * 蔚来签到 = type=cron,cronexp=0 7 * * *,wake-system=1,script-path=https://raw.githubusercontent.com/emoemperor/Surge/main/scripts/nio/check_in.js
 */
const NAME = "蔚来签到";
const BASE_URL = "https://gateway-front-external.nio.com";
const PREFIX = "emo_";
const TOKEN_KEY = `${PREFIX}nio_token`;
const UPDATE_KEY = `${PREFIX}nio_token_update_at`;
const APP_ID = "10086";
const USER_AGENT = "Mozilla/5.0 (iPhone; CPU iPhone OS 18_7 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 NIOAppCN/5.48.5 (com.do1.WeiLaiApp; build:2549; OS:iOS) webview/lg _dsbridge";
...
function buildURL(params) {
    ...
    return `${BASE_URL}/moat/10086/c/award_cn/checkin?${queryString}`;
}
...
!(async () => {
    ...
    const cookie = $.getval(TOKEN_KEY);
    const updatedAt = $.getval(UPDATE_KEY);
    if (!cookie || !updatedAt || Date.now() - parseInt(updatedAt) > 7 * 24 * 60 * 60 * 1000) {
        $.msg(NAME, "⚠️请先获取cookie");
        return;
    } else {
        $.msg(NAME, "开始签到");
        await check_in(cookie);
    }
})()
    ...
    await $.post(options, (err, resp, data) => {
        ...
        try {
            ...
            if (result.result_code === "success") {
                const checkinInfo = result.data.stats;
                const tip = result.data.tip;
                $.msg(NAME, `签到成功`, `连续签到 ${checkinInfo.continuous_checkin_days} 天`, tip);
                $.log("签到成功，服务器返回:", JSON.stringify(result));
            } else {
                $.log("签到失败，服务器返回:", JSON.stringify(result));
                $.msg(NAME, "签到失败", `服务器返回: ${result.result_code}`);
            }
        } catch (e) {
            $.msg(NAME, "签到失败", "响应解析错误");
            $.logErr(e);
        }
    });
}
